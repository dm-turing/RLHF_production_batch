...
// Publish sends a message to all subscribers, with panic recovery.
...
// Subscriber function receives messages and processes them, with panic recovery.
		...
		defer func() {
			if r := recover(); r != nil {
				logrus.WithFields(logrus.Fields{
					"error":      fmt.Sprintf("%v", r),
					"subscriber": id,
				}).Error("Recovered in subscriber - panic occurred.")
			}
		}()

		if msg.Content == "panic" { // Simulate a panic on a specific message
			var nilPointer *int
			_ = *nilPointer // This will trigger a panic
		}
...
func main() {
	...
	messages := []Message{
		...
		{Content: "panic"}, // This will cause a panic in subscriber
